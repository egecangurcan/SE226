...
vector<string> words = {"a", "ability", "able", "about", "above", "accept", "according", "account", "across", "act",
                        ...
                        "answer", "any", "anyone", "anything", "appear", "apply", "approach", "area", "argue",
                        ...
                        "blood", "blue", "board", "body", "book", "born", "both", "box", "boy", "break", "bring",
                        ...
                        "clear", "clearly", "close", "coach", "cold", "collection", "college", "color", "come",
                        ...
                        "during", "each", "early", "east", "easy", "eat", "economic", "economy", "edge",
                        "education", "effect", "effort", "eight", "either", "election", "else", "employee", "end",
                        ...
                        "not", "note", "nothing", "notice", "now", "n't", "number", "occur", "of", "off", "offer",
                        "office", "officer", "official", "often", "oh", "oil", "ok", "old", "on", "once", "one",
                        "only", "onto", "open", "operation", "opportunity", "option", "or", "order", "organization",
                        ...
                        "recent", "recently", "recognize", "record", "red", "reduce", "reflect", "region", "relate",
                        ...
                        "role", "room", "rule", "run", "safe", "same", "save", "say", "scene", "school", "science",
                        ...
                        "shot", "should", "shoulder", "show", "side", "sign", "significant", "similar", "simple",
                        ...
                        "someone", "something", "sometimes", "son", "song", "soon", "sort", "sound", "source",
                        ...
                        "turn", "TV", "two", "type", "under", "understand", "unit", "until", "up", "upon", "us",
                        "use", "usually", "value", "various", "very", "victim", "view", "violence", "visit",
                        "voice", "vote", "wait", "walk", "wall", "want", "war", "watch", "water", "way", "we",
...
string passGen(int user_number)
{
    ...
    vector<string> random_words(shuffled_words.begin(), shuffled_words.begin() + user_number);
    string password;
...
string rep_with_upper(string password)
{
    random_device rd;
    mt19937 gen(rd());
    vector<char> char_indices;
    for (int i = 0; i < password.length(); i++) {
        char_indices.push_back(i);
    }
    if (char_indices.size() > 0) {
        uniform_int_distribution<> dist(0, char_indices.size() - 1);
        int random_index = char_indices[dist(gen)];
        char upper_char = toupper(password[random_index]);
        replace(password.begin(), password.end(), password[random_index], upper_char);
    }
    return password;
}
...
string swap_letters(string password)
{
    ...
    char second_char = password[1];
    char second_last_char = password[password.length() - 2];
    char last_char = password[password.length() - 1];
    password[0] = second_last_char;
    password[1] = last_char;
    password[password.length() - 2] = first_char;
    password[password.length() - 1] = second_char;
    return password;
}

string search_letter(string source,char key){
    for ( char i : source){
        if (i == key){
            return "True";
        }
...
{
    int user_number;
    cout << "Please enter a number between 3 and 7, inclusive: ";
    cin >> user_number;
    while (user_number < 3 || user_number > 7) {
        cout << "Invalid number. Please enter a number between 3 and 7, inclusive: ";
        cin >> user_number;
    }
    string password = passGen(user_number);
    string final_password = swap_letters(rep_with_upper(password));
    ...
    cin >> first_letter;
    cout << search_letter(final_password, first_letter) << endl;
